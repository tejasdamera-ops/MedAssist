...
export function Login() {
  ...
  } = useForm({ resolver: zodResolver(schema), defaultValues: { email: "admin@medassist.test", password: "Password123!" } });

  async function onSubmit(values) {
    try {
      const user = await login(values);
      const targetHome = roleHome[user.role] || "/";
      const fromPath = location.state?.from?.pathname;
      const isValidRedirect = fromPath && fromPath !== "/403" && fromPath !== "/login" && fromPath.startsWith(targetHome);
      navigate(isValidRedirect ? fromPath : targetHome, { replace: true });
    } catch (error) {
  ...
  return (
    <main className="grid min-h-screen place-items-center px-4">
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md rounded-md border border-clinic-line bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-clinic-teal">MedAssist</h1>
        <p className="mt-1 text-sm text-slate-600">Clinic Operations & Patient Care Portal</p>
        ...
        <p className="mt-4 text-center text-sm text-slate-600">
          New patient?{" "}
          <Link className="font-medium text-clinic-teal hover:underline" to="/register">
            Create an account
          </Link>
        </p>
      </form>
